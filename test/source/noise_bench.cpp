        ...
        for (int64_t x = 0; x < size_x; x++) {
            for (int64_t y = 0; y < size_y; y++) {
                for (int64_t z = 0; z < size_z; z++) {
                    benchmark::DoNotOptimize(perlin.octave3D_01(static_cast<double>(x) / 256.0, static_cast<double>(y) / 256.0, static_cast<double>(z) / 256.0, octaves, persistence) * lacunarity);
