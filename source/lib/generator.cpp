...
generator::generator(uint32_t _seed)
    : seed(_seed)
{
    perlin.reseed(seed);
}
...
uint32_t generator::randomize_seed()
{
...
uint32_t generator::get_seed()
{
    return seed;
}

void generator::generate_2d_heightmap(std::vector<uint32_t>& heightmap,
                                      const int begin_x,
                                      const int begin_y,
                                      const int begin_z,
                                      ...
                                      const uint32_t size_z)
{
    ...
    const int end_x = static_cast<int>(begin_x + size_x);
    const int end_y = static_cast<int>(begin_y + size_y);
    const int end_z = static_cast<int>(begin_z + size_z);
...
void generator::generate_3d_heightmap(std::vector<uint32_t>& heightmap,
                                      const int begin_x,
                                      const int begin_y,
                                      const int begin_z,
                                      const uint32_t size_x,
...
std::vector<chunk> generator::generate_word(
    const int begin_chunk_x, const int begin_chunk_y, const int begin_chunk_z, const uint32_t chunk_x, const uint32_t chunk_y, const uint32_t chunk_z)
{
    constexpr bool debug = false;
...
void generator::generate_word(std::vector<chunk>& chunks,
                              ...
                              const bool generate_3d_terrian)
{
    constexpr bool debug = false;
    ...
    for (uint32_t x = 0; x < chunk_x; x++) {
        for (uint32_t z = 0; z < chunk_z; z++) {
            for (uint32_t y = 0; y < chunk_y; y++) {
                const int real_x = static_cast<int>(x) + begin_chunk_x;
                const int real_y = static_cast<int>(y) + begin_chunk_y;
                const int real_z = static_cast<int>(z) + begin_chunk_z;
...
void generator::generate_2d(std::vector<block>& blocks,
                            const int begin_x,
                            const int begin_y,
                            const int begin_z,
                            const uint32_t size_x,
...
void generator::generate_3d(std::vector<block>& blocks,
                            const int begin_x,
                            const int begin_y,
                            const int begin_z,
                            const uint32_t size_x,
